...
```
There seems to exist a linear relationship between BMI and weight, as weight increase the BMI also changes.
# Correlation plot
...
```
From the above correlation plot, we can see that bmi, weight, apache 3j diagnosis, elective surgery, icu admit source, height, apache 2 diagnosis are highly correlated variables.
...
```
apache 2 diagnosis lies in the range 110- 300, the median is at 120, it doesn't have any outliers.
apache 3j diagnosis lies in the range 210- 700, the median is at, it has many outliers in the third quartile.
heart rate apache lies between 80 to 120 and has few outliers
map apache lies between 55 and 125, its median is at 60 and it doesn't have any outliers.
resprate apache lies in the range 11 and 35, the median is at 28 and it does not have any outliers.
...
From the optimal parameter result, we can see that k with optimal parameter as 12 has not made much improvement in the result. **Therefore the chance of an admitted patient surviving in a hospital is 70%**  
...
**We consider the over sampled data for runing logistic regression model**
```{r}
# Logistic regression
...
**From the above model led summary statistics, it shows that all the apache predictors we selected are statistically significant**
...
**For in sample prediction, the logistic regression model results with the balanced accuracy score of 62% and sensitivity score of 59%, a specificity score of 65% **
...
**The final logit model with out of sample prediction gives the accuracy score of 62% which is the same for in sample data. Giving us altogther the prediction of an admitted patient's survival in the hospital as 62%**
...
In terms of balanced accuracy, the decision tree using the undersampling data performs
the best. The oversampled tree is about the same, but the normal sample trailed behind.
This helps affirm that balancing the data will help the models predict more accurately.
...
```
For the decision tree models that did not consider the calculated Apache death 
probabilities, undersampling also performed the best overall.

## Simple Decision Trees to Predict Apache Hospital Death Probability

```{r}
train_data_wo_icu <- as.data.frame(subset(train_data,select = -apache_4a_icu_death_prob))
over_wo_icu <- as.data.frame(subset(over,select = -apache_4a_icu_death_prob))
under_wo_icu <- as.data.frame(subset(under,select = -apache_4a_icu_death_prob))

first_tree_hosp <- tree(apache_4a_hospital_death_prob ~ ., data = train_data_wo_icu)
summary(first_tree_hosp)
plot(first_tree_hosp)
text(first_tree_hosp, cex = 0.75, col = 'red')

oversampled_tree_hosp <- tree(apache_4a_hospital_death_prob ~ ., data = over_wo_icu)
summary(oversampled_tree_hosp)
plot(oversampled_tree_hosp)
text(oversampled_tree_hosp, cex = 0.75, col = 'red')

undersampled_tree_hosp <- tree(apache_4a_hospital_death_prob ~ ., data = over_wo_icu)
summary(undersampled_tree_hosp)
plot(undersampled_tree_hosp)
text(undersampled_tree_hosp, cex = 0.75, col = 'red')

ft_hosp_yhat <- predict(first_tree_hosp, newdata = test_data)
over_tree_hosp_yhat <- predict(oversampled_tree, newdata = test_data)
under_tree_hosp_yhat <- predict(undersampled_tree, newdata = test_data)

pr_ft_hosp <- postResample(ft_hosp_yhat, test_data$apache_4a_hospital_death_prob)
pr_over_tree_hosp <- postResample(over_tree_hosp_yhat, test_data$apache_4a_hospital_death_prob)
pr_under_tree_hosp <- postResample(under_tree_hosp_yhat, test_data$apache_4a_hospital_death_prob)

print("Decision tree balanced accuracy:")
pr_ft_hosp
print("Oversampled tree balanced accuracy:")
pr_over_tree_hosp
print("Undersampled tree balanced accuracy:")
pr_under_tree_hosp
```

The first decision tree has the highest Rsquared, so it is mostly likely the best 
fit and prediction model for the hospital death probability among the three decision
trees.

## Simple Decision Trees to Predict Apache Hospital Death Probability

```{r}
train_data_wo_hosp <- as.data.frame(subset(train_data,select = -apache_4a_hospital_death_prob))
over_wo_hosp <- as.data.frame(subset(over,select = -apache_4a_hospital_death_prob))
under_wo_hosp <- as.data.frame(subset(under,select = -apache_4a_hospital_death_prob))

first_tree_icu <- tree(apache_4a_icu_death_prob ~ ., data = train_data_wo_hosp)
summary(first_tree_icu)
plot(first_tree_icu)
text(first_tree_icu, cex = 0.75, col = 'red')

oversampled_tree_icu <- tree(apache_4a_icu_death_prob ~ ., data = over_wo_hosp)
summary(oversampled_tree_icu)
plot(oversampled_tree_icu)
text(oversampled_tree_icu, cex = 0.75, col = 'red')

undersampled_tree_icu <- tree(apache_4a_icu_death_prob ~ ., data = under_wo_hosp)
summary(undersampled_tree_icu)
plot(undersampled_tree_icu)
text(undersampled_tree_icu, cex = 0.75, col = 'red')

ft_icu_yhat <- predict(first_tree_icu, newdata = test_data)
over_tree_icu_yhat <- predict(oversampled_tree_icu, newdata = test_data)
under_tree_icu_yhat <- predict(undersampled_tree_icu, newdata = test_data)

pr_ft_icu <- postResample(ft_icu_yhat, test_data$apache_4a_icu_death_prob)
pr_over_tree_icu <- postResample(over_tree_icu_yhat, test_data$apache_4a_icu_death_prob)
pr_under_tree_icu <- postResample(under_tree_icu_yhat, test_data$apache_4a_icu_death_prob)
...
pr_under_tree_icu
```
The first decision tree also has the highest Rsquared when trying to predict the 
ICU death probability among the three decision trees.
...
library(caret)
set.seed(1234)
ann_trainIndex <- createDataPartition(data$apache_4a_icu_death_prob, p = .8, list = FALSE)

ann_train_set <- data[ ann_trainIndex,]
ann_test_set <- data[-ann_trainIndex,]
...
Using neuralnet() method to train the neural network model with 5 hidden layers.
```{r}
library(neuralnet)

f <- as.formula(apache_4a_icu_death_prob ~ icu_admit_source + icu_stay_type + icu_type + pre_icu_los_days + apache_2_diagnosis + apache_3j_diagnosis + gcs_eyes_apache + gcs_motor_apache + gcs_verbal_apache + heart_rate_apache + map_apache + resprate_apache + temp_apache + d1_diasbp_max + d1_diasbp_min + d1_heartrate_max + d1_heartrate_min + d1_mbp_max + d1_mbp_min + d1_resprate_max + d1_resprate_min + d1_spo2_max + d1_spo2_min + d1_sysbp_max + d1_sysbp_min + d1_temp_max + d1_temp_min + h1_diasbp_max + h1_diasbp_min + h1_heartrate_max + h1_heartrate_min + h1_mbp_max + h1_mbp_min + h1_resprate_max + h1_resprate_min + h1_spo2_max + h1_spo2_min + h1_sysbp_max + h1_sysbp_min)

# Fit a neural network model with 2 hidden layers
nn_fit_2 <- neuralnet(f, data = train_scaled, hidden = 3, linear.output=TRUE) # 45- 50 mins
